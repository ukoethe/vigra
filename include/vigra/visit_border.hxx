...
struct visit_border_impl
{
    ...
    static void exec(const MultiArrayView<N, Data, S1>& u_data, MultiArrayView<N, Label, S2>& u_labels,
                     ...
                     NeighborhoodType neighborhood, const Shape& difference, Visitor visitor)
    {
        ...
        else if(difference[D] == 0)
        {
            ...
            next::exec(u_data, u_labels, v_data, v_labels, difference, visitor);
        }
        else
        {
...
struct visit_border_impl<0>
{
                     ...
                     NeighborhoodType neighborhood, const Shape& difference, Visitor visitor)
    {
        if(neighborhood == DirectNeighborhood)
        {
            ...
            typedef typename MultiArrayView<N, Label, S2>::const_iterator LabelsIterator;
            ...
            {
                visitor(*u_data, *u_labels, *v_data, *v_labels);
            }
        } 
        else if(neighborhood == IndirectNeighborhood)
        {
            ...
            for(GraphScanner node(graph); node != lemon::INVALID; ++node)
            {
                ...
                {
                    visitor(u_data[*node], u_labels[*node], v_data[g.target(*arc)], v_labels[g.target(*arc)]);
                }
            }
        }
    }
