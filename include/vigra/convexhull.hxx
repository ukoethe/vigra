...
//******* This maybe should be in another file ***********
...
public:
	inline
	bool operator()(TinyVector<float, 2> const & a,
			TinyVector<float, 2> const & b) const {
 ...
 */
void pushLinePoints(TinyVector<float, 2> p1, TinyVector<float, 2> p2,
		std::vector<TinyVector<float, 2> > & result) {

	TinyVector<float, 2> ps, diff = p2 - p1;
	bool yLonger = false;
	int incVal, endVal;
	float shortLen = diff[0];
	float longLen = diff[1];
	...
	if (yLonger) {
		...
		for (; abs(i) < abs(endVal); i += incVal, j += decInc)
			result.push_back(TinyVector<float, 2>(p1[0] + i, p1[1] + j));
	} else {
		int i = incVal;
		float j = decInc;
		for (; abs(i) < abs(endVal); i += incVal, j += decInc)
			result.push_back(TinyVector<float, 2>(p1[0] + j, p1[1] + i));
	}
 ...
 */
template<class T>
void fillPolygon(std::vector<TinyVector<float, 2> > p,
	...
	pushLinePoints(*(p.end() - 1), *(p.begin()), contour_points);
	...
	float min_y = (*(contour_points.begin()))[0];
	float max_y = (*(contour_points.end() - 1))[0];

	while (points_iterator != contour_points.end()) {
		float y = (*points_iterator)[0];
		if ((y > min_y) && (y < max_y)) { // We are inside the polygon
			TinyVector<float, 2> current = *points_iterator;
			float min_x = current[1];
			current[0] = ceil(current[0]);
			current[1] = ceil(current[1]);

			bool drawing = true;

			while ((*points_iterator)[0] == y) {
				++points_iterator;

				TinyVector<float, 2> endPoint = *points_iterator;
				float max_x = endPoint[1];

				// Draw the scan line

				for (; current[1] < endPoint[1]; ++current[1]) {
					if ((current[1] > min_x) && (current[1] < max_x)) {
						if (drawing) {
 ...
 */
void extractContour(MultiArray<2, int> label_image,
		TinyVector<int, 2> anchor_point,
		std::vector<TinyVector<float, 2> > &contour_points) {

	TinyVector<int, 2> neightbour_coords[4] = { TinyVector<int, 2>(-1, 0),
			TinyVector<int, 2>(0, 1), TinyVector<int, 2>(1, 0), TinyVector<int,
					2>(0, -1) };

	TinyVector<int, 2> directions[4] = { TinyVector<int, 2>(0, -1), TinyVector<
			int, 2>(-1, 0), TinyVector<int, 2>(0, 1), TinyVector<int, 2>(1, 0) };
	...
	bool contour_travelled = false;
	int visited_initial_position = 0;
	// Go around the object
	while (!contour_travelled) {

		// Go forward until we don't touch the wall or there is a right turn
		while (true) {

			// We have to visit twice the initial position, once when we start, and another time when we finish
			if (position == initial_position) {
				visited_initial_position++;
			}

			// We have finished
			if (visited_initial_position > 1) {
				contour_travelled = true;
				break;
			}
			...
			if (label_image(position[0], position[1]) == 1) {
				// Go back and...
				position -= directions[direction];
 ...
 */
bool findAnchorPoint(MultiArray<2, int> input_image,
		TinyVector<int, 2> &anchor_point) {
		...
		for (unsigned x = 1; x < input_image.width() - 1; ++x) {
			if (input_image(y, x)) {
				for (unsigned i = 0; i < 4; ++i) {
					unsigned neighbour_y = y + neightbour_coords[i][0];
					unsigned neighbour_x = x + neightbour_coords[i][1];
					// It must have at least a background pixel
					if (!input_image(neighbour_y, neighbour_x)) {
						anchor_point = TinyVector<int, 2>(y, x);
						return true;
					}
				}
			}
...
public:
	...
	ConvexHullFeatures(MultiArray<2, int> input_image) :
			featuresCalculated(false) {
		calculateFeatures(input_image);

	}

	void calculateFeatures(MultiArray<2, int> input_image) {
		...
		AccumulatorChain<int, Select<Sum> > convexHullAccumulator;

		extractFeatures(convex_hull_image.begin(), convex_hull_image.end(),
				convexHullAccumulator);

		convexHullArea = get<Sum>(convexHullAccumulator);

		AccumulatorChain<int, Select<Sum> > inputImageAccumulator;

		extractFeatures(input_image.begin(), input_image.end(),
				inputImageAccumulator);

		inputArea = get<Sum>(inputImageAccumulator);
