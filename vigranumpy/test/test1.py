...
def test_watersheds():
	...
	assert_raises(RuntimeError, watersheds, img_scalar_f, 5, img_scalar_i)
...
def test_region_growing():
	res = seededRegionGrowingSeeded2D(img_scalar_f,img_scalar_i)
	checkShape(img_scalar_f,res)

def test_seededRegionGrowing2D():
	res = seededRegionGrowing2D(img_scalar_f)
	checkShape(img_scalar_f,res)
	checkType(res,np.int32)
def test_MinimaMaxima():
	res = localMinima2D(img_scalar_f)
	checkShape(img_scalar_f,res)
	checkType(res,img_scalar_f.dtype)
	
	res = extendedLocalMinima2D(img_scalar_f)
	checkShape(img_scalar_f,res)
	checkType(res,img_scalar_f.dtype)
	
	res = localMaxima2D(img_scalar_f)
	checkShape(img_scalar_f,res)
	checkType(res,img_scalar_f.dtype)
	
	res = extendedLocalMaxima2D(img_scalar_f)
	checkShape(img_scalar_f,res)
	checkType(res,img_scalar_f.dtype)
