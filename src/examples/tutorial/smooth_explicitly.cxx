...
int main (int argc, char ** argv) 
{
    ...
    try {
        ...
        {   
            ...
            MultiArray<2, float> resultImage(info.shape());
            
            // Read the input data.
            importImage(info, destImage(inputImage));

            // Smooth image by calculating the mean of a 3x3 window around each pixel.
            ...
            for(current[1] = 0; current[1] < inputImage.shape(1); ++current[1])
            {
                for (current[0] = 0; current[0] < inputImage.shape(0); ++current[0])
                {
                    ...
                    // Note:
                    //    * Near the image border, not all pixels in a 3x3 window are within
                    ...
                    //      window. Therefore, we have to add 'Shape2(2)' (a vector of 2s) to 'current'. 
                    //      In contrast, the upper left corner 'windowStart' must be inside the window, 
                    //      so we only subtract 'Shape2(1)' (a vector of 1s) from 'current'. 
                    Shape2 windowStart = max(Shape2(0),          current - Shape2(1));
                    Shape2 windowStop  = min(inputImage.shape(), current + Shape2(2));
            ...
            exportImage(srcImageRange(resultImage), ImageExportInfo(argv[2]));
        }
