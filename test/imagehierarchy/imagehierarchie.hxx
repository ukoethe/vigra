...
class VariableBandsImage                          ////////////////////////VariableBandsImage, davon erben sie alle
{
...
//    typedef ... ConstScanOrderIterator;
    ...
    virtual ~VariableBandsImage() {}              /////////////////////Destroctor
    ...
    Diff2D size() const                                 //////////size
        { return roiLR_ - roiUL_; }
    Diff2D actualSize() const                           /////////actualSize
        { return actualLR_; }
    int actualWidth() const                             /////////actualWidth
        { return actualSize().x; }
    int actualHeight() const                            /////////actualWidth
        { return actualSize().y; }
    int actualBands() const                             /////////actualBands
        { return actualBands_; }
    int bands() const                                   /////////bands
        { return bands_; }
    Diff2D roiUpperLeft() const                         /////////roiUpperLeft
        { return roiUL_; }
    
    
    virtual void setROI(Diff2D const & ul, Diff2D const & new_size)        ////////////////virtual  setROI???????
    {
    ...
    PixelType operator[](Diff2D const & c)                          //////////////////->wie gewohnt 
  ...
  protected:
    VariableBandsImage()                                  /////////////////Konstruktoren sind wiedermal protected
    : data_(0)
    {}
    
    VariableBandsImage(VariableBandsImage const & s)                     /////////////Object
    : roiUL_(s.roiUL_), roiLR_(s.roiLR_), actualLR_(s.actualLR_),
      bands_(s.bands_), actualBands_(s.actualBands_),
      data_(s.data_)
    {}
    
    VariableBandsImage & operator=(VariableBandsImage const & s)               //////////Zuweisungsoperator
    {
        roiUL_ = s.roiUL_;
        roiLR_ = s.roiLR_;
    ...
    void init(GrayValue * data, Diff2D const & actualLR, int actualBands)         ///////////init (GrayValue <- float Zahl
    {
    ...
    static GrayValue * getDataPtr(GrayValue * p)          ///////////////wofuer ist das??????????????
        { return p; }
    ...
    GrayValue * get(int const & dx, int const & dy)                  //////////////get Funktion gibt Gray value
        { return data_ + ((roiUL_.y + dy) * actualWidth() + roiUL_.x + dx)*actualBands_; }
    
    GrayValue const * get(int const & dx, int const & dy) const      //////////////////dasselbe in const
        { return data_ + ((roiUL_.y + dy) * actualWidth() + roiUL_.x + dx)*actualBands_; }
...
class FixedBandsImage                                     /////////////FixedBandsImage erbt von VariableBandsImage
: public VariableBandsImage
{
  public:
    ...
    FixedBandsImage(int w, int h)                                   ////////////Konstruktoren int
    : VariableBandsImage(),
    ...
    FixedBandsImage(Diff2D const & s)                                      ///////////////Diff2D
    : image_(new InnerImage(s))
    {
        init(getDataPtr(image_->begin()), image_->size(), BandsForPixelType<PixelType>::bands);
    }
    
    FixedBandsImage(FixedBandsImage const & s)                            //////////////////Object
    : VariableBandsImage(s),
      image_(s.image_)
    {}
    
    FixedBandsImage(InnerImage * i)                                 ///////////////////wieder InnerImage von SelectImage
    : image_(i)
    {
        init(getDataPtr(image_->begin()), image_->size(), BandsForPixelType<PixelType>::bands);
    }
    
    FixedBandsImage & operator=(FixedBandsImage const & s)                /////////////Zuweisungsoperator
    {
        if(this != &s)
        {
            image_ = s.image_;
    ...
    virtual ~FixedBandsImage()          ///////////////warum ist Destruktor virtual
        {}
    
    virtual CloneType * clone() const             ////////////virtual clone    
    {
    ...
    virtual FixedBandsImage * shallowCopy() const           ////////////virtual shallowCopy
    {
...
class FixedRGBImage                                   /////////////////////FixedRGBImage erbt von FixedBandsImage
: public FixedBandsImage<IMAGE, VectorAccessor<TinyVector<GrayValue, 3> > >
{
  public:
    ...
    FixedRGBImage(int w, int h)                ///////Konstruktoren int
    : BaseType(w,h)
    {}
    
    FixedRGBImage(Diff2D const & s)             /////////Diff2D
    : BaseType(s)
    {}
    
    FixedRGBImage(FixedRGBImage const & s)           //////////Object
    : BaseType(s)
    {}
    
    FixedRGBImage(InnerImage * i)                   //////////InnerImage
    : BaseType(i)
    {}
    
    FixedRGBImage & operator=(FixedRGBImage const & s)          /////////Zuweisungsoperator
    {
        BaseType::operator=(s);
        return *this;
    }
    ...
    virtual FixedRGBImage * shallowCopy() const                   //////////virtual shallowCopy
    {
    ...
    ConstAccessor accessor() const                              ////accessor const
        { return ConstAccessor(); }
};
...
class SingleBandImage                           //////////////////////////////////SingleBandImage erbt von
: public VariableBandsImage                             //////////////////////        VariableBandImage
{
  public:
...
//    typedef ... const_iterator; 
    ...
    virtual ~SingleBandImage()                             ///////////////////Destruktor
        {}
    
    virtual CloneType * clone() const = 0;                 ///////////////////virtual clone()
    virtual SingleBandImage * shallowCopy() const = 0;     ///////////////////virtual shallowCopy()
    
    PixelType & operator[](Diff2D const & c)              /////////////////////////////operrator[Diff2D]
        { return *get(c.x, c.y); }

    PixelType const & operator[](Diff2D const & c) const
        { return *get(c.x, c.y); }

    PixelType & operator()(int x, int y)                     /////////////////////operator()(int,int)
        { return *get(x, y); }

    PixelType const & operator()(int x, int y) const
        { return *get(x, y); }

    Iterator upperLeft()                          //////////////////////////////upperLeft
        { return Iterator(get(0,0), actualWidth(), actualBands_); }
        
    Iterator lowerRight()                         ///////////////////////////////lowerRirght    
        { return upperLeft()+size(); }
        
    ConstIterator upperLeft() const                  //////////////////////////////upperLeft const
  ...
  protected:                                       //////////////////////////Konstruktoren
    ...
    SingleBandImage & operator=(SingleBandImage const & s)     ////////////////////////////////operator=
    {
        VariableBandsImage::operator=(s);
        return *this;
    }
...
class GrayImage                                            /////////////////////////GrayImage erbt von SingleBandImage
: public SingleBandImage
{
  public:
    ...
    GrayImage(int w, int h)                                        //////////////////Konstruktoren->1
    : image_(new InnerImage(w,h))
    {
        init(getDataPtr(image_->begin()), image_->size(), 1);
    }
    
    GrayImage(Diff2D const & s)                                   ///////////////////2  Diff2D
    : image_(new InnerImage(s))
    {
        init(getDataPtr(image_->begin()), image_->size(), 1);
    }
    
    GrayImage(GrayImage const & s)                                ///////////////////Object
    : SingleBandImage(s),
      image_(s.image_)
    {}
    
    GrayImage(InnerImage * i)                                     //komisch InnerImage kommt aus SelectBandImage
    : image_(i)
    {
        init(getDataPtr(image_->begin()), image_->size(), BandsForPixelType<PixelType>::bands);
    }
    
    GrayImage & operator=(GrayImage const & s)                 //////////operator=
    {
        if(this != &s)
        {
            image_ = s.image_;
    ...
    virtual ~GrayImage()                                         /////////////////////VIRTUELLE SACHEN->
        {}
...
class GrayImage;   /////////????????????????????///////////wofuer?

class SelectBandImage                                  //////////////////////SelectBandImage erbt von SingleBandImage
: public SingleBandImage
{
    class ImageHandle
    {
    ...
    : public ImageHandle
    {
        boost::shared_ptr<IMAGE> ptr_;              //////////////was ist das?
  ...
  public:
    ...
    template <class IMAGE>                                 //////Das sieht wie Konstruktor aus  int
    SelectBandImage(IMAGE const & s, int band)
    : imageHandle_(new ImageHandleImpl<typename IMAGE::InnerImage>(s.image_)),    /////was ist imageHandle_
      band_(band)
    {
    ...
    SelectBandImage(SelectBandImage const & s)        //////Das sieht wie Konstruktor aus  Object
    : SingleBandImage(s),
      imageHandle_(s.imageHandle_),
      band_(s.band_)
    {}

    SelectBandImage & operator=(SelectBandImage const & s)              /////////Zuweisungsoperator
    {
        if(this != &s)
        {
            imageHandle_ = s.imageHandle_;
    ...
    virtual ~SelectBandImage()                   ////Destruktor ist wieder mal virtuel
        {}
    ...
    virtual SelectBandImage * shallowCopy() const             /////virtual shallowCopy
    {
        return new SelectBandImage(*this); 
    }
    
    virtual void setROI(Diff2D const & ul, Diff2D const & new_size)       /////////////////setROI's
    {
        VariableBandsImage::setROI(ul, new_size);
    }
    
    virtual void setROI(int band)                                               ///////////////noch mal setROI/////////
    {
