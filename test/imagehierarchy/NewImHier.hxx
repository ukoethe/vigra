...
*/
    ...
    static bool less(BaseType const& d1, BaseType const& d2)
    {
        return d1.current.y == d2.current.y ? d1.current.x < d2.current.x : d1.current.y < d2.current.y;
    }
...
public:
    ...
    */
    virtual void setROI(Diff2D const& ul, Diff2D const& new_size) // setROI -methode mit Argumenten Diff2D upperLeft und Diff2D size
    {
        ...
        Diff2D new_lr;
        new_lr.x = new_size.x > 0 ? new_ul.x + new_size.x : roiLR_.x + new_size.x;
        new_lr.y = new_size.y > 0 ? new_ul.y + new_size.y : roiLR_.y + new_size.y;
...
{
public:
    typedef FixedBandsImage<IMAGE, VectorAccessor<TinyVector<GrayValue, 3>>> BaseType;
